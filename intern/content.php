...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tab === 'hero') {
        ...
        // Zusätzlich buchbare Optionen (Preisliste auf der Vereinshaus-Seite)
        $_optLabels = $_POST['opt_label'] ?? [];
        $_optPrices = $_POST['opt_price'] ?? [];
        $_optUnits  = $_POST['opt_unit']  ?? [];
        $_opts = [];
        foreach ($_optLabels as $_i => $_lbl) {
            $_lbl = trim((string)$_lbl);
            if ($_lbl === '') continue;
            $_opts[] = [
                'label' => $_lbl,
                'price' => max(0, (float)str_replace(',', '.', $_optPrices[$_i] ?? '0')),
                'unit'  => trim((string)($_optUnits[$_i] ?? '€')) ?: '€',
            ];
        }
        $c['booking_options'] = $_opts;
...
$bookingOpts  = (isset($c['booking_options']) && is_array($c['booking_options']) && $c['booking_options'])
    ? $c['booking_options']
    : [
...
<div class="tab-panel <?= $activeTab === 'prices' ? 'active' : '' ?>" id="tab-prices">
  <form method="POST">
    ...
    <div class="card">
      ...
      <p style="font-size:0.85rem;color:#5a6c5a;margin-bottom:20px">Diese Preisliste erscheint auf der Vereinshaus-Seite unter „Zusätzlich buchbare Optionen". Zeile ohne Bezeichnung wird beim Speichern gelöscht.</p>
      <div id="opt-rows" style="display:flex;flex-direction:column;gap:10px">
        <?php foreach ($bookingOpts as $_o): ?>
        <div class="opt-row" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <input type="text"   name="opt_label[]" value="<?= htmlspecialchars((string)($_o['label'] ?? '')) ?>" placeholder="Bezeichnung" style="flex:1 1 220px">
          <input type="number" name="opt_price[]" value="<?= htmlspecialchars((string)($_o['price'] ?? 0)) ?>" min="0" step="0.01" style="width:90px">
          <input type="text"   name="opt_unit[]"  value="<?= htmlspecialchars((string)($_o['unit'] ?? '€')) ?>" placeholder="€" style="width:80px">
          <button type="button" onclick="this.closest('.opt-row').remove()" style="background:none;border:none;color:#c62828;cursor:pointer;font-size:1.1rem">✕</button>
        </div>
        <?php endforeach; ?>
      </div>
      <button type="button" onclick="var r=document.querySelector('#opt-rows .opt-row').cloneNode(true);r.querySelectorAll('input').forEach(function(i){i.value=i.name==='opt_unit[]'?'€':''});document.getElementById('opt-rows').appendChild(r)" style="margin-top:12px;background:#eef4e6;border:1px solid #cddcbf;border-radius:6px;padding:6px 12px;cursor:pointer">＋ Option hinzufügen</button>
    </div>
      ...
      <div style="border-top:1px solid #e0ead6;margin:20px 0;padding-top:20px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          ...
          <div class="field">
            ...
            <input type="text" name="settings_email" value="<?= htmlspecialchars($settings['email'] ?? 'vorstand@example.org') ?>" maxlength="120" placeholder="kontakt@example.org">
          </div>
        </div>
      </div>
      <div style="border-top:1px solid #e0ead6;margin:20px 0;padding-top:20px">
